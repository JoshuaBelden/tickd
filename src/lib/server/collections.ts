import type { List, Tag, Task, UserSettings } from "$lib/types"
...
export async function getTagsCollection(): Promise<Collection<Tag & Document>> {
  const db = await getDb()
  return db.collection("tags") as Collection<Tag & Document>
}
