<script lang="ts">
  ...
  let newTaskListId = $state(untrack(() => data.lists[0]?._id ?? ""))
  ...
  $effect(() => {
    ...
    if (!taskId) return
    goto(`/all`, { replaceState: true, noScroll: true })
  ...
  let groupedTasks = $derived.by(() => {
    ...
    if (groupBy === "list") {
      return data.lists.map((l: any) => ({
        ...
        color: l.color,
        isDone: false,
        tasks: filteredTasks.filter(t => t.listId === l._id && !t.parentId),
      }))
    }
  ...
  async function createQuickTask(title: string, status: string) {
    if (!title.trim() || !data.lists[0]) return
    const res = await fetch("/api/tasks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function archiveAllDone() {
    // Archive done tasks across all lists
  ...
  async function createSubtask(parentId: string, title: string) {
    ...
    const res = await fetch("/api/tasks", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        listId: parent?.listId ?? data.lists[0]?._id,
        title,
  ...
  const listMap = $derived(
    new Map(data.lists.map((l: any) => [l._id, l]))
  )

  function getListForTask(task: Task) {
    return listMap.get(task.listId)
  }
</script>
  ...
  <div class="flex-1 flex flex-col overflow-hidden min-w-0">
    ...
    <header class="flex flex-wrap items-center gap-2 px-4 py-3 border-b border-border flex-shrink-0">
      <h1 class="font-semibold text-lg flex items-center gap-2 mr-1">
        ...
        <span class="truncate max-w-[160px] sm:max-w-none">All Tasks</span>
      </h1>
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center px-4 py-1.5 text-xs text-gray-500 border-b border-border bg-background sticky top-0 z-10">
            <div class="flex-1 flex items-center gap-2">
              {#if sortBy === "order"}<div class="w-4 flex-shrink-0"></div>{/if}
              <div class="w-2.5 flex-shrink-0"></div>
              <span>Task</span>
            </div>
            <div class="hidden sm:block w-28 flex-shrink-0">List</div>
                ...
                {#each group.tasks as task (task._id)}
                  ...
                  >
                    <TaskCard
                      ...
                      onUpdate={updateTask}
                      onDelete={deleteTask}
                      listName={getListForTask(task)?.name}
                      listColor={getListForTask(task)?.color}
                  ...
                  {#if $showSubtasksInList}
                    {#each tasks.filter(t => t.parentId === task._id) as subtask (subtask._id)}
                      <TaskCard
                        ...
                        onUpdate={updateTask}
                        onDelete={deleteTask}
                        listName={getListForTask(subtask)?.name}
                        listColor={getListForTask(subtask)?.color}
                        indent={true}
                      />
                    {/each}
                  {/if}
  ...
  >
    <div class="card w-full max-w-md space-y-4" onclick={e => e.stopPropagation()} role="presentation">
      ...
      <div class="flex gap-3">
        <div class="flex-1">
          <label class="text-xs text-gray-400 block mb-1" for="new-task-list">List</label>
          <select
            id="new-task-list"
            class="text-xs bg-surface border border-border rounded px-2 py-1 text-gray-300 w-full"
            bind:value={newTaskListId}
          >
            {#each data.lists as l (l._id)}
...
{#if overlayTask ?? selectedTask}
  <TaskDetail
    ...
    {statusConfig}
    {allTags}
    currentListId={selectedTask?.listId ?? overlayTask?.listId ?? ""}
